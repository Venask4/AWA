...
var exp = (function($) {
	...
	exp.init = function() {
		...
		function addShipping() {
			...
			// // Fix Northern Ireland postal code bug - Removed per IPSL shipping changes 4/18/17
			...
			// 	$('.validate-one-required-by-name').first().click();
			// };

			// Simulate click on shipping method to allow order to place
			var newShip = $('#onestepcheckout-shipping-method-section-cloned').text();
			if (newShip !== exp.vars.oldShip) {
				$('#onestepcheckout-shipping-method-section-cloned').find('label').first().click();
				exp.vars.oldShip = $('#onestepcheckout-shipping-method-section-cloned').text();
			}
